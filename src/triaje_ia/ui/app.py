...
# ─────────────────────────────────────────────────────────────
def _render_fase_1_legacy():
    st.markdown('<span class="sec-label">Entrada guiada de triaje</span>', unsafe_allow_html=True)

    with st.form("triage_intake_form", border=False):

        # ── Sección: Paciente ──
        st.markdown(
            '<div class="form-section-title">'
            '<span class="required-dot"></span> Datos del paciente'
            '</div>',
            unsafe_allow_html=True,
        )
        edad_col, sexo_col = st.columns([1, 1], gap="small")
        with edad_col:
            edad_raw = st.text_input(
                "Edad *",
                placeholder="años",
            )
        with sexo_col:
        ...
        # ── Sección: Motivo y síntomas ──
        st.markdown(
            '<div class="form-section-title">'
            '<span class="required-dot"></span> Motivo y síntomas'
            '</div>',
            unsafe_allow_html=True,
        )
        motivo_consulta = st.text_input(
            "Motivo principal o síntoma guía *",
            placeholder="p. ej. dolor torácico, disnea, fiebre...",
        )
        duracion_sintomas = st.text_input(
            "Duración / evolución",
            placeholder="p. ej. 2 horas, desde ayer, progresivo",
        )
        sintomas_frecuentes = st.pills(
            "Síntomas frecuentes",
            SINTOMAS_FRECUENTES,
        ...
        sintomas_adicionales = st.text_input(
            "Otros síntomas o hallazgos",
            placeholder="Separados por comas: náuseas, sudoración, tos...",
        )
        ...
        st.markdown(
            '<div class="form-section-title">'
            '<span class="required-dot"></span> Constantes vitales y dolor'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        # ── Sección: Contexto clínico (colapsable) ──
        st.markdown(
            '<div class="form-section-title">Contexto clínico</div>',
            unsafe_allow_html=True,
        )
        antecedentes = st.text_input(
            "Antecedentes relevantes",
            placeholder="Separados por comas: EPOC, cardiopatía, diabetes...",
        )
        medicacion = st.text_input(
            "Medicación habitual",
            placeholder="Separada por comas: anticoagulantes, insulina...",
        )
        observaciones = st.text_area(
            "Observaciones libres",
            placeholder="Aspecto general, mecanismo lesional, datos que no encajan en campos anteriores...",
            height=90,
        )
        ...
        datos_formulario = TriageFormData(
            ...
            sintomas_frecuentes=sintomas_frecuentes or [],
        ...
        avisos_formulario = avisos_numericos + validar_entrada_triaje(datos_formulario)

        # Mínimos obligatorios: edad + sexo + motivo + ≥1 constante
        tiene_constante = any([
        ...
        minimos_ok = (
            edad is not None
            and bool(sexo)
            and bool((motivo_consulta or "").strip())
            and tiene_constante
            and not avisos_numericos
        )
    ...
    # ── Post-submit: validación y transición ──
    if analizar:
        ...
        # Errores de formato en constantes
        for aviso in avisos_numericos:
            errores_bloqueo.append(aviso)

        if errores_bloqueo:
            for err in errores_bloqueo:
                st.error(err)
        else:
            # Avisos no bloqueantes (solo informativos)
            avisos_info = validar_entrada_triaje(datos_formulario)
            for aviso in avisos_info:
                st.markdown(
            ...
            # Transición a Fase 2
            narrativa = generar_narrativa_triaje(datos_formulario)
            st.session_state.datos_formulario = datos_formulario
            st.session_state.ultima_narrativa = narrativa.strip()
            st.session_state.fase = 2
            st.rerun()

    st.markdown("""<p style="font-size:0.72rem; color:#94A3B8; margin-top:0.9rem; line-height:1.75; text-align:center;">El formulario genera una narrativa estructurada para el modelo. No sustituye el criterio profesional.</p>""", unsafe_allow_html=True)


# ─────────────────────────────────────────────────────────────
# FASE 2 — PANTALLA DE PROCESAMIENTO
# ─────────────────────────────────────────────────────────────
def _render_fase_1():
...
def _render_fase_2_legacy():
    # Scroll to top al entrar en fase de procesamiento
    ...
    # Container visual de procesamiento
    container = st.empty()
    progress_bar = st.progress(0)

    def _update_display(step_index: int, progress_pct: float):
        steps_html = _render_processing_steps_html(step_index)
        container.markdown(
            '<div style="display:flex; flex-direction:column; align-items:center; justify-content:center;'
            ' min-height:55vh; text-align:center; padding:2.5rem 1.5rem;'
            ' background:linear-gradient(165deg,#FAFBFC 0%,#EFF4FB 50%,#F5F7FA 100%);'
            ' border-radius:16px; max-width:580px; margin:0 auto;">'
            '<div style="display:flex; align-items:center; gap:0.7rem; margin-bottom:1.5rem;">'
            '<span style="font-family:var(--font-sans); font-size:1.5rem; font-weight:700; color:#1F2D3D;">'
            'tr<span style="color:var(--btn-bg); font-weight:800;">IA</span>je</span></div>'
            '<div style="font-family:var(--font-sans); font-size:1.1rem; font-weight:700;'
            ' color:#1F2D3D; margin-bottom:1.5rem;">Analizando caso cl\u00ednico</div>'
            f'<div style="text-align:left; width:100%; max-width:340px;">{steps_html}</div>'
            '</div>',
            unsafe_allow_html=True,
        )
    ...
    try:
        # Paso 0: Preparar narrativa
        _update_display(0, 0.08)
        time.sleep(0.4)

        # Paso 1: Enviar al LLM
        _update_display(1, 0.18)
        time.sleep(0.3)

        # Paso 2: Extraer vector
        _update_display(2, 0.35)
        vector = _extraer_fn(narrativa, modelo=MODELO_LLM_FIJO)
        st.session_state.ultimo_vector = vector

        # Paso 3: Validar coherencia
        _update_display(3, 0.55)
        time.sleep(0.3)

        # Paso 4: Calcular predicción
        _update_display(4, 0.70)
        predictor = _cargar_predictor()
        result = predictor.predict(vector, narrativa)
        st.session_state.ultimo_resultado_ml = result

        # Paso 5: Generar SHAP
        _update_display(5, 0.88)
        time.sleep(0.3)

        # Paso 6: Completado
        _update_display(6, 1.0)
        time.sleep(0.5)
        ...
        if st.button("← Volver al formulario", type="secondary"):
            st.session_state.fase = 1
            st.rerun()
